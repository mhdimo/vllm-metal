...
class TestDSparkLifecycle:
    ...
    def test_release_requests_drops_per_request_state(self) -> None:
        """Evicted/preempted requests must not keep holding drafter context.

        The runner's lifecycle reconcile calls ``release_requests`` on every
        drafter (the scheduler-managed cache group contract); missing it is an
        AttributeError that takes EngineCore down mid-serve.
        """
        p = _proposer()
        p._ctx_caches["r0"] = [object()]
        p._n_cached["r0"] = 4
        p._pending_seed["r0"] = mx.zeros((2, 8))
        p._ctx_caches["r1"] = [object()]
        p._n_cached["r1"] = 2

        p.release_requests({"r0"})

        assert "r0" not in p._ctx_caches
        assert "r0" not in p._n_cached
        assert "r0" not in p._pending_seed
        assert p._n_cached["r1"] == 2  # other requests stay untouched
