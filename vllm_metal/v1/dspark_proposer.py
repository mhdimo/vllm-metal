...
class DSparkProposer:
    ...
    def release_requests(self, req_ids: set[str]) -> None:
        """Drop per-request drafter state for evicted/preempted requests.

        The drafter's context KV grows per request with no eviction of its own,
        so a parked request must release its context here (the scheduler-managed
        cache group contract on :class:`MetalProposer`); a resumed request
        re-seeds from the tapped target forward.
        """
    ...
    def _batch_draft(
        ...
        mx.eval(draft_arr)
        # mlx's tolist() is untyped upstream; numpy's stub types it as a scalar
        # union, so pin the real shape ([B, cap] -> list of per-row token lists).
        rows: list[list[int]] = cast("list[list[int]]", draft_arr.tolist())
        return (
