...
def validate_dspark_config(config: VllmConfig, *, use_paged_attention: bool) -> None:
    ...
    if not use_paged_attention:
        raise NotImplementedError("Metal DSpark requires paged attention")
    ...
    if envs.VLLM_USE_V2_MODEL_RUNNER is not False:
        raise NotImplementedError(
            "Metal DSpark uses the Metal V1 runner. Set VLLM_USE_V2_MODEL_RUNNER=0 "
            "to disable upstream's GPU-only DSpark runner selection."
        )
