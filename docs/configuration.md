...
| `VLLM_METAL_SMALL_M_QMM` | `auto` | Small-M quantized matmul dispatch for affine 4-bit layers (group 64, input width a multiple of 512). `mx.quantized_matmul` leaves its weight-streaming path at six rows for a GEMM tiling that is badly utilized below about sixteen, so speculative verification rows (`num_speculative_tokens + 1` per request), the DSpark drafter's block rows and 6-32 concurrent decodes pay up to 2.2x a single row. `auto` measures every eligible weight shape once at load (numerics against the stock kernel, then a dependent-chain race at 8, 16, 24 and 32 rows; verdicts cached per GPU architecture and MLX version under `~/.cache/vllm-metal/small-m/`) and routes 6-32-row calls through a kernel that dequantizes each weight group once per threadgroup where it wins by 10% or more; `off` keeps the stock kernel everywhere; `on` skips the race. Outputs differ from the stock kernel at the bfloat16 ULP level (fp32 accumulation in another order), the class the parity contract records as ties. LoRA and pooling serves keep the stock kernel. |
...
| `VLLM_METAL_DECODE_PIPELINE` | `1` | One-step-ahead decode sampling pipeline: eligible pure-decode greedy steps defer the sampling sync one step so the next step's graph build and submit overlap the in-flight GPU forward. Greedy output is unchanged. With DSpark configured (which runs under the asynchronous scheduler) the pipeline runs on steps the proposer will not draft at (the `bypass` mode, or the `adaptive` planner's bypass decision for the batch; the drafter keeps ingesting target features on such steps); the other speculative methods force synchronous scheduling and keep it off. Set to `0` to force the fully synchronous per-step sample path. |
...
| `VLLM_METAL_NATIVE_SAMPLING` | `0` | Opt-in MLX-native non-greedy sampling for the decode pipeline: the pipeline's deferred sampler learns a temperature/top-k/top-p graph (`mx.random.categorical` over a top-k/top-p mask), so eligible non-greedy pure-decode steps defer their sampling sync like greedy ones instead of dropping to the synchronous torch path. Requests with seeds, penalties, logprobs, or token constraints keep the torch path (`min_p` and `logit_bias` are rejected at request validation on Metal, as before). Sampled tokens come from the MLX RNG keyed off the engine seed, so outputs differ from the torch sampler's stream. Off by default while the path gathers serve mileage; set to `1` to enable. |
