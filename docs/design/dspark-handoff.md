...
| M6: confidence calibration and adaptive planning | Complete on M5 Max for the pinned 4B pair: confidence recording with censored survival labels, sequential temperature scaling with per-position reliability, the measured cost model, the causal prefix planner with its oracle tests, and the adaptive serving mode (`VLLM_METAL_DSPARK_MODE=adaptive`) with draft-or-not bypass, per-request history and counters, evaluated against target-only and fixed K (see the [progress record](dspark-progress.md)). |
| M7: production performance and reliability | Done on M5 Max within the recorded scope: the HTTP soak (`tools/dspark_soak.py`, fixed mode at K=7, mixed workload with cancellations) ran for 65 minutes and 13,289 requests without a failure and with a flat memory trajectory; the serving cost is profiled (context, the affine-4 multi-row matmul path, the bypass floor, the serving path's fifth) and the optimization targets are recorded, not changed. Packaged deployment stays DEFERRED: no Xcode Metal compiler on this machine. |
| M8: additional standalone model pairs | Open; each pair needs its own adapter, precision, capacity and serving gates. |
| M9: performance parity with the official runners | Done on M5 Max: drafter context arena with per-row attention, batched feature ingest, `DeferredStepProposer` seam (inert until M9b), target KV budget fix (153 -> 1,006 blocks at the bench limits), drafter precision knob, acceptance evaluation against the paper (within 0.1 of Table 1 on gsm8k/math500/mt-bench/alpaca, 0.3 on the code sets at T=1.0), verify-window mode measured and left off, contamination-aware paired benches. Served K=7 step -9..-21% (cost-04 vs cost-03); adaptive vs target-only: +50% to +60% at one client on short prompts, +28% to +34% on 512-token outputs, +3% to +9% on 1,024-token prompts, and -1% to -9% at four to sixteen clients where the planner bypasses (M6b: -6% to -20%). Serving gates fixed/adaptive/bypass and the stochastic gate pass (`results/m5max-m9-final-01`, `-02`). |
| M9b: DSpark under asynchronous scheduling | Done on M5 Max: the platform keeps vLLM's asynchronous scheduler for `dspark` and the runner fills the scheduler's placeholder slots from its retained drafts (upstream's contract); the M9 deferred-step seam engages on non-drafting steps. Every server with `--async-scheduling`: serving gates fixed/adaptive/bypass pass, 20-minute soak passes, adaptive bench 128x128 C1 +31.4% against the asynchronous target-only server (13% faster than the synchronous one; the DSpark server is unchanged because a drafting step still synchronizes), bypass floor -2.5..-4.3% at 4-8 clients; the stochastic gate's one failing cell is the target's own batch-shape spread (two control runs). Next lever: GPU-resident verification and draft handoff. |
| M9e: load regime | Done on M5 Max: the proposer lapses after 32 consecutive planner declines (contexts and slots released, no feature capture, bookkeeping only) and resumes after 4 would-draft verdicts; bypass lapses from the start. Evidence (`results/m5max-m9e-lapse-01`, asynchronous scheduler): serving gates adaptive and bypass pass; adaptive bench at 4/8/16 clients with the lapse on against off: 128x128 +0.9/+0.2/+1.1% (off -5.5/-4.2/-7.7), 128x512 +2.9/+3.1/-0.5% (off -9.5/+4.7/-4.0), 1024x128 -4.7/-13.4/-12.3% (off -4.0/-2.4/-11.6; the eight-client cell alternates with pair order and is open); ten-minute adaptive soak at eight clients: PASS (1,507 requests, zero errors, 184.7 tok/s, resident set flat, one lapse entry and one exit). |
| M9c: small-M quantized matmul for verification rows | Measured and NOT adopted on M5 Max: an 8x8-MMA small-M kernel (per-shape numerics check and race at load) built, tested and probed; the stock GEMV streams weights faster at one row than any MMA layout at eight, so the in-model gain at eight rows is 1.03x on the MLP and 1.16x on the head, no step-level gain; code kept on `dspark-m9c-small-m`, investigation in the progress record. Lead kept: MLX 0.32.2's `qmm_t_nax` 32-row block (1.3-1.6x at 16-32 rows in the probe; served trial half done, pin unchanged at 0.32.1). |
...
| M6: calibrated planning | M6a done on M5 Max: `vllm_metal/v1/dspark/calibration.py` (recorder with censored survival labels, STS on a fixed grid, ECE with bootstrap intervals, Brier, reliability bins, manifest-checked artifact), `tools/dspark_confidence_calibrate.py` (record on a revisioned corpus split by prompt, fit, report), `vllm_metal/v1/dspark/planner.py` (cost model interpolating measured target/draft/host costs with bounds, causal prefix planner with strict early stopping and deterministic ties, draft-or-not decision with reasons) and `tools/dspark_cost_profile.py`; tests include a brute-force oracle. M6b done on M5 Max: `vllm_metal/v1/dspark/adaptive.py` binds both artifacts behind `VLLM_METAL_DSPARK_MODE=adaptive` (explicit failure without them), decides before the backbone whether the batch drafts (bypass reasons counted), cuts each row to the planned prefix, resets history on release and idle steps, and keeps `DSparkCounters`; `bypass` is the explicit never-draft mode. The cost model became a serving-path profile over request counts, widths and decode contexts (`dspark-cost/2`) after the in-process profile under-predicted the served step. Evaluation against target-only and fixed K in the progress record. Remaining: repeat on the M4 machine; the bypass floor (a speculative server that never drafts is slower than a target-only server because the decode pipeline is off and features are captured) is the M7 optimization target. |
| M7: production hardening | Done on M5 Max within the recorded scope: `tools/dspark_soak.py` ran the HTTP soak in the fixed mode at K=7, the demanding path (mixed greedy/sampled requests, budgets 8-256, long and shared-prefix prompts, streaming and non-streaming, client cancellations) for 65 minutes and 13,289 completed or cancelled requests with no failure, an idle server at the end, continuous draft work and a flat memory trajectory (progress record); the proposer logs its counters every 2,000 drafting steps; `tools/dspark_matmul_rows_probe.py` attributes the multi-row verification cost. Packaged deployment (`scripts/test.sh` wheel) stays DEFERRED: it needs Xcode's Metal compiler, absent here; every run used source-built kernels. Optimization targets recorded, not changed: the bypass floor (decode pipeline off and feature capture with speculative decoding configured) and the affine-4 multi-row matmul path. |
...
| M9: performance parity | Done on M5 Max for the pinned 4B pair within the recorded scope: the drafter's contexts live in one per-layer arena (`ContextArena`, slots reserved at load with the block's scratch positions), a drafting batch attends every row to a strided view of its own slot (no padding, gather or mask) and a step's accepted rows are ingested in one batched pass per layer; the `DeferredStepProposer` seam lets the decode pipeline run on steps the proposer will not draft (bypass and adaptive-bypass steps), implemented and unit-tested but inert in serving until DSpark runs under asynchronous scheduling (M9b); the target KV planner no longer double-counts the arena nor reserves three context copies (the speculative server's cache went from 153 to 1,006 blocks at the bench's limits); the verification kernel's window mode was measured and left off; a drafter precision knob and an accepted-length evaluation against the paper's Table 1. Evidence: in-process breakdowns (`results/m5max-m9-ab-01`), the cost-04 serving profile and the adaptive/fixed benches at C=1/4/8/16 (`results/m5max-m9-final-01`), serving and stochastic gates in both modes. Remaining: GPU-resident verification and draft handoff so drafting steps join the asynchronous pipeline (M9d); the MLX 0.32.2 quantized GEMM path for 16-32 verification rows (probe 1.4-2x on the head and MLP shapes; trialled as a separate dependency change); the small-M kernel was measured and not adopted (`dspark-m9c-small-m`); repeat every gate on the M4 machine. |
| M9b: asynchronous scheduling | Done on M5 Max for the pinned 4B pair: the Metal platform keeps vLLM's asynchronous scheduler for `dspark` (still synchronous for draft-model, MTP and n-gram); the runner retains each request's drafts and fills the scheduler's placeholder slots at execution (`SpeculativeDecodeController.substitute_retained_drafts`), unused slots reported so the metrics count real drafts, and the controller validates the runner's own resolution; the deferred-step seam of M9 engages on non-drafting steps. Evidence (`results/m5max-m9b-async-01`, every server with `--async-scheduling`): serving gates in the fixed, adaptive and bypass modes, the stochastic gate, adaptive and fixed benches at C=1/4/8 against the asynchronous target-only server, and a 20-minute fixed-K=7 soak. Remaining: GPU-resident verification and draft handoff so drafting steps join the pipeline (the 13% the asynchronous target-only server gained at one client); repeat every gate on the M4 machine; M9c (small-M quantized matmul for the verification rows) follows on this branch's head. |
| M9e: the load regime | Done on M5 Max: `VLLM_METAL_DSPARK_LAPSE` (default on); adaptive bench at 4/8/16 clients with the lapse on against off, serving gates in the adaptive and bypass modes, a 10-minute adaptive soak at eight clients. Remaining: GPU-resident verification and draft handoff so drafting steps join the pipeline (M9d); repeat every gate on the M4 machine. |
| M9c: verification rows on a small-M kernel | Not adopted (measured, no step-level gain on this GPU; `results/m5max-m9c-smallm-01`). Remaining: finish the MLX 0.32.2 served trial (`results/m5max-mlx0322-01`: benches, adaptive/bypass gates, soak; re-run its cost profile on a quiet machine) after adding an explicit margin to the paged-attention allowance in the cache policy; bump the pin only if every gate passes and the 16-32-row cells improve. Then the M8 pairs, and GPU-resident verification and draft handoff as the next milestone. |
