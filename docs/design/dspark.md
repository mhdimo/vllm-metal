...
| `quant/small_m.py` | Small-M quantized matmul (M9c): verification rows, the drafter's block rows and 6-32-row decodes run on a kernel that dequantizes each weight group once per threadgroup, enabled per weight shape by a numerics check and a race at load (`VLLM_METAL_SMALL_M_QMM`) | Keep the stock kernel the reference for numerics; every enabled shape must have measured faster on this GPU and MLX version. |
