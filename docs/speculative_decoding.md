...
On a Mac (32 GB) with `mlx-community/Qwen3-4B-4bit` +
`deepseek-ai/dspark_qwen3_4b_block7`, single-stream, warm, 512-token decode:

- Baseline (no spec): ~48 tok/s.
- DSpark (K=2): ~74 tok/s — **~1.5×** (mean acceptance ~1.2/round, ~59%).

The speedup is masked by the cold start (kernel compilation + one-time
prompt seeding) on short first requests; measure warm and with long-enough
generation to amortize the seed.
