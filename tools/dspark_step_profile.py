...
not a serving benchmark (``vllm bench serve``).
...
from tools.dspark_tooling import model_ref
...
def worker(config: dict, output: Path) -> None:
    ...
    from tools.dspark_tooling import NATURAL_PROMPTS
...
def main() -> None:
    ...
    env = dict(
        os.environ,
        # In-process engine so the runner and proposer can be instrumented.
        VLLM_ENABLE_V1_MULTIPROCESSING="0",
        # Synchronous sampling on every step so K=0 phases are attributable;
        # serving keeps the production pipeline default.
