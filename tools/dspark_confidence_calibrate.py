...
"""Record DSpark confidence outcomes on the real pair and fit STS.
...
from tools.dspark_tooling import (
    CORPUS_FILES,
    corpus_windows,
    git_revision,
    model_ref,
...
def main() -> None:
    ...
    rec.add_argument("--gpu-memory-utilization", type=float, default=0.3)
    ...
    if args.command == "record":
        if args.worker_config:
            ...
            return
        config = {
            "target": model_ref(args.target),
            "draft": model_ref(args.draft),
        ...
        # In-process engine so the recorder can attach to the proposer.
        env = dict(os.environ, VLLM_ENABLE_V1_MULTIPROCESSING="0")
